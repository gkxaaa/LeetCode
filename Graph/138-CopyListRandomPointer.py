...
class Solution(object):
    ...
    思路2：dfs，停止边界是当前节点被访问过或者下一个点为空，每次访问当前节点时，搞定cur.next或cur.random然后标记cur。但有一个问题，若先访问next节点一直到头发现有环，那么递归将不会停止maximum recursion depth exceeded.所以另一个边界是每个节点的next或者random都被访问过。
    '''
