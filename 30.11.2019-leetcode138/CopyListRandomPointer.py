...
class Solution(object):
    '''
    一道类似于有向图遍历的题。
    思路1: bfs，每次遍历当前节点的next和random后并标记当前节点，然后对next和random节点重复操作
         （注意如果next和random节点是一个节点的话操作一次）。用队列存放接下来还需要遍历的节点，若队列空说明所有节点都被访问。
    思路2：dfs，停止边界是当前节点被访问过或者下一个点为空，每次访问当前节点时，搞定cur.next或cur.random然后标记cur。
    '''
